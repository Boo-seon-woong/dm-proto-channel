...
## 규약 (v3.1 — admin 제정: 역할 · 30분 하트비트 · 커밋 감지 메커니즘 · 단일 대화 흐름)

> v3는 admin이 제정했다 (channel.md 2026-07-06 07:40 UTC `[admin]` 항목).
> v3.1은 Rule 3(커밋 감지)를 세션-독립 OS-레벨 메커니즘으로 개정했다
> (channel.md 2026-07-06 `[admin]` v3.1 항목). admin 지시는 아래 모든 규칙에 우선한다.
...
### Rule 3 — 커밋 감지·응답 메커니즘 (v3.1: 세션-독립 OS-레벨, 양측 공통)

감지 메커니즘은 Claude 세션이 아니라 **OS cron이 소유**한다. 세션이 죽거나(터미널
종료·리부트), usage limit으로 턴이 막히거나, 사용자 선택지 대기로 블록돼도 감지는
계속 돌고, 장애가 풀리면(limit 초기화 등) 응답이 **자동 재개**된다.
...
- **wake 전달 2경로**:
  ...
  2. *slow path* — pending이 3분 넘게 방치되면 watcher가
     ...
     권한은 `--allowedTools`로 git·파일편집만 부여. 성공 시 포크된 새 session_id를
     기록해 다음 wake가 직전 wake의 기억을 이어받는다.
