...
#
# Env overrides: VSIZES, MIXES, CLIENTS, THREADS, TEST_TIME, RUNS, KEYMAX
set -u
...
CLIENTS=${CLIENTS:-8}; THREADS=${THREADS:-1}; TEST_TIME=${TEST_TIME:-30}; RUNS=${RUNS:-3}
KEYMAX=${KEYMAX:-16384}
...
echo "# $CONFIG -> $SERVICE_IP:$SERVICE_PORT  (vsizes: $VSIZES; mixes: $MIXES; c$CLIENTS t$THREADS ${TEST_TIME}s x$RUNS)" >> "$CMDS"
...
for mix in $MIXES; do
  ...
  for vsize in $VSIZES; do
    ...
    for run in $(seq 1 "$RUNS"); do
      ...
      cmd="memtier_benchmark -s $SERVICE_IP -p $SERVICE_PORT -P memcache_text --ratio=$ratio --data-size=$vsize --clients=$CLIENTS --threads=$THREADS --test-time=$TEST_TIME --key-maximum=$KEYMAX --hide-histogram"
